...
function Button({
  ...
  const baseClasses = 'relative px-8 py-3 rounded-lg font-semibold text-sm transition-all duration-300 overflow-hidden group';
    ...
    >
      ...
      <motion.div
        className={`absolute inset-0 rounded-lg border-2 opacity-0 ${
          variant === 'primary' ? 'border-teal-400' : 'border-white/40'
        }`}
        whileHover={{
          opacity: [0, 0.6, 0],
          scale: [1, 1.1, 1.2],
